...
//            locationManager.showsBackgroundLocationIndicator = false
    ...
    func startMonitoringForEntryAtPlace(place: HyperTrackPlace, radius:CLLocationDistance, identifier:String){
        ...
        if let placeLocation = place.location{
            ...
            HTLogger.shared.info("starting monitoring for region having identifier : " + identifier  + " radius : " + clampedRadius.description)
            let monitoringRegion = CLCircularRegion(center: (placeLocation.toCoordinate2d()), radius: clampedRadius, identifier: identifier)
            HTLogger.shared.info("startMonitorForPlace having identifier: \(identifier ) ")
...
extension LocationManager: CLLocationManagerDelegate {
                        ...
                        if let circularRegion = monitoringRegion as? CLCircularRegion{
                            if(circularRegion.contains(clLocation.coordinate)){
                                if circularRegion.notifyOnEntry {
                                    HTLogger.shared.info("entered region due to a location update , identifier : " + monitoringRegion.identifier)
                                    if let locationEventDelegate = self.locationEventsDelegate{
                                ...
                                HTLogger.shared.info("exited region due to a location update , identifier : " + monitoringRegion.identifier)
    ...
    public func locationManager(_ manager: CLLocationManager, monitoringDidFailFor region: CLRegion?, withError error: Error){
        if (region != nil){
            HTLogger.shared.info(" location manager monitoringDidFailFor " + (region?.identifier)!)
        }
    }
}
